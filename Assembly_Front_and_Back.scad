...
module Front_Bar_Calibration_face(){
  translate([0, -RightPanel_basewidth/2 + bar_cut_length, base_bars_Zdistance + base_bars_height+30]){
  rotate([90,0,0]){
  difference(){
  rounded_square([SidePanels_distance+100,60], corners=[5,5,20,20], center=true);
#frente();
}    
}
  }
...
//sara
module Rear_Bar_Calibration_face(){
...
//sara
module Front_Bar_Calibration_face_sheet(){
  BillOfMaterials(category="Lasercut wood", partname="XEnd Idler Plain Face");
  material("lasercut")
  //translate([- thickness - XEnd_box_size, -XPlatform_width/2 + 1.5*thickness, thickness])
  //rotate([90,0,0])
  linear_extrude(height=thickness)
  Front_Bar_Calibration_face();
}

module Rear_Bar_Calibration_face_sheet(){
  BillOfMaterials(category="Lasercut wood", partname="XEnd Idler Plain Face");
  material("lasercut")
  //translate([- thickness - XEnd_box_size, -XPlatform_width/2 + 1.5*thickness, thickness])
  //rotate([90,0,0])
  linear_extrude(height=thickness)
  Rear_Bar_Calibration_face();
}
module FrontBars(){
...
module Calibration_Panels(){
  Front_Bar_Calibration_face_sheet();
  Rear_Bar_Calibration_face_sheet();
}
